...
public class PrefixService {
	...
	public void delete(BigInteger id) {
		int count = repository.deleteByIdAndPrefixIsNotNull(id);

		if(count == 0) throw new SystemFailedException("Prefix not deleted");
	}
	...
	public boolean existsByIdAndUser(BigInteger prefixId, BigInteger userId) {
		if(!repository.existsByIdAndUserId(prefixId, userId)) throw new EntityNotFoundException("Prefix not found");
		return true;
	}

	public boolean existsByIdAndAuth(BigInteger prefixId) {
		if(!repository.existsByIdAndUserId(prefixId, AuthenticatedUser.getUserId())) throw new EntityNotFoundException("Prefix not found");
		return true;
	}
